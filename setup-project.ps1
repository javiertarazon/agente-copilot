...
$repoUrl = "https://github.com/javiertarazon/agente-copilot.git"
...
function Resolve-AgentPath {
    ...
    $clone = Read-Host "Clonar desde GitHub en '$env:USERPROFILE\agente-copilot'? [S/n]"
    if ($clone -eq "n") {
        throw "Instalacion cancelada."
    }
    $destination = "$env:USERPROFILE\agente-copilot"
